...
export function AuthProvider({ children }) {
  ...
  const value = useMemo(() => ({ user, loading, login: async (identifier, password) => { const result = await loginRequest(identifier, password); setUser(result.user); setAuthenticatedUser(result.user); return result.user; }, logout: async () => { await logoutRequest().catch(() => {}); logoutUser(); setUser(null); } }), [user, loading]);
