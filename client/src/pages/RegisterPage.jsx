...
export default function RegisterPage() {
  const navigate = useNavigate(); const { register } = useAuthContext(); const [form, setForm] = useState({ email: "", username: "", password: "", name: "" }); const [message, setMessage] = useState("");
  async function submit(event) { event.preventDefault(); try { await register(form); navigate("/student-profiling", { replace: true }); } catch (error) { setMessage(error.response?.data?.error || "Could not create account."); } }
