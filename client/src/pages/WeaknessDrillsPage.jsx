...
const PAGE_FONT_FAMILY = "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif";
...
export default function WeaknessDrillsPage() {
  ...
  if (!analysis.hasAttempts) {
    return (
      <div className="min-h-screen bg-white p-6" style={{ fontFamily: PAGE_FONT_FAMILY }}>
        <GlobalStyle />
        <div className="relative z-10 mx-auto flex min-h-[80vh] max-w-3xl flex-col items-center justify-center text-center">
          ...
          <section className="relative mt-10 w-full overflow-hidden rounded-3xl border border-slate-200 bg-white/80 p-10 shadow-[0_10px_40px_-15px_rgba(15,23,42,0.15)] backdrop-blur-xl">
            <span className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full bg-blue-50 blur-3xl" />
            <span className="pointer-events-none absolute -bottom-10 -left-10 h-40 w-40 rounded-full bg-slate-100 blur-3xl" />
  ...
  if (activeSubject) {
    ...
    return (
      <div className="min-h-screen bg-white p-5 md:p-10" style={{ fontFamily: PAGE_FONT_FAMILY }}>
        <GlobalStyle />
          ...
          {!questions.length ? (
            ...
            <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
              ...
              <article className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-[0_20px_60px_-20px_rgba(15,23,42,0.2)] backdrop-blur-xl transition-all duration-300 md:p-10">
                <span className="pointer-events-none absolute right-0 top-0 h-16 w-16 border-b border-l border-slate-200" style={{ clipPath: "polygon(100% 0, 100% 100%, 0 0)" }} />
                <span className="pointer-events-none absolute -right-16 -top-16 h-48 w-48 rounded-full bg-slate-50 blur-3xl" />
            ...
            <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
              <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white/90 p-8 text-center shadow-[0_20px_60px_-20px_rgba(15,23,42,0.2)] backdrop-blur-xl lg:sticky lg:top-6 lg:h-fit">
                <span className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-slate-100/70 to-transparent" />
                <div className="relative">
                  <span className="relative mx-auto grid h-20 w-20 place-items-center rounded-full border border-blue-200 bg-blue-50 text-4xl text-blue-600 shadow-[0_0_35px_-6px_rgba(37,99,235,0.4)]">
                    <FaTrophy />
                    <span className="absolute inset-0 -z-10 animate-ping rounded-full border border-blue-300/60" />
                  </span>
                  <p className="mt-4 font-mono text-[11px] font-black uppercase tracking-[0.25em] text-blue-700">Round Complete!</p>
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-[0_2px_10px_-4px_rgba(15,23,42,0.08)]"><p className="font-mono text-xs font-black uppercase tracking-wide text-slate-500">Best Streak</p><p className="mt-1 text-3xl font-black text-slate-900">{results.bestStreak}</p></div>
                  <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5 shadow-[0_2px_10px_-4px_rgba(15,23,42,0.08)]"><p className="font-mono text-xs font-black uppercase tracking-wide text-blue-700">Points Earned</p><p className="mt-1 text-3xl font-black text-blue-900">{results.points}</p></div>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-[0_2px_10px_-4px_rgba(15,23,42,0.08)] backdrop-blur-xl">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">{results.items.map((item, index) => <span key={index} className={`inline-flex items-center gap-1 rounded-full px-3 py-1 font-mono text-xs font-black ${item.isCorrect ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-slate-200 bg-slate-100 text-slate-600"}`}>{item.isCorrect ? <FaCheckCircle /> : <FaTimesCircle />} {item.isCorrect ? "Correct" : "Incorrect"}</span>)}</div>
                </div>
  ...
  function renderQuestionInput(question, index) {
          ...
          {question.choiceOpts.map((option, optionIndex) => (
            <button
              ...
              className={`flex items-center gap-3 rounded-xl border px-4 py-3 text-left text-sm font-bold transition-all duration-200 ${
                responses[index] === optionIndex ? "border-blue-300 bg-blue-50 text-blue-700 shadow-[0_0_16px_-4px_rgba(37,99,235,0.35)]" : "border-slate-200 bg-slate-50 text-slate-700 hover:border-slate-300 hover:bg-slate-100"
          ...
          {question.choiceOpts.map((option, optionIndex) => {
            ...
            return (
              <button
                ...
                disabled={Boolean(results)}
                className={`flex items-center gap-3 rounded-xl border px-4 py-3 text-left text-sm font-bold transition-all duration-200 ${
                  selected ? "border-blue-300 bg-blue-50 text-blue-700 shadow-[0_0_16px_-4px_rgba(37,99,235,0.35)]" : "border-slate-200 bg-slate-50 text-slate-700 hover:border-slate-300 hover:bg-slate-100"
                } ${results ? "cursor-not-allowed opacity-70" : ""}`}
              >
    ...
    return (
      <input
        ...
        className="mt-4 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-800 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
...
function DiagnosticReport({ analysis, adaptiveGate }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-[0_2px_10px_-4px_rgba(15,23,42,0.1)] backdrop-blur-xl">
      <span className="pointer-events-none absolute -right-12 -top-12 h-36 w-36 rounded-full bg-blue-50 blur-3xl" />
      <p className="relative font-mono text-[11px] font-black uppercase tracking-[0.2em] text-blue-700">Performance Diagnostic</p>
      ...
      {diagnostic && (
        <div className="relative mt-4 inline-flex flex-col rounded-xl border border-blue-200 bg-blue-50 px-4 py-2">
...
function AdaptiveGatePanel({ gate, status }) {
  return (
    <section className="relative mb-6 overflow-hidden rounded-3xl border border-blue-100 bg-white/90 p-6 shadow-[0_10px_40px_-18px_rgba(15,23,42,0.15)] backdrop-blur-xl sm:p-8">
      <span className="pointer-events-none absolute -left-16 -top-16 h-52 w-52 rounded-full bg-blue-50 blur-3xl" />
      <span className="pointer-events-none absolute -bottom-20 -right-16 h-52 w-52 rounded-full bg-slate-100 blur-3xl" />
      <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <span className="relative grid h-14 w-14 shrink-0 place-items-center rounded-2xl border border-blue-200 bg-blue-50 text-blue-600">
            <FaRoute className="text-xl" />
            {status !== "idle" && <span className="absolute inset-0 animate-ping rounded-2xl border border-blue-300/70" />}
          </span>
          <div>
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <p className="text-[11px] font-black uppercase tracking-[0.2em] text-blue-700">Previous Results</p>
      </div>
      <div className="mt-3 space-y-2">
        {sessions.slice(0, 5).map((session, index) => {
          ...
          return <div key={session?.id || `${session?.subject}-${index}`} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5 transition-colors duration-200 hover:bg-slate-50">
            ...
            <span className={`shrink-0 rounded-full px-2 py-1 text-xs font-black ${score >= 75 ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-700"}`}>{score}%</span>
          </div>;
        })}
      </div>
    </section>
